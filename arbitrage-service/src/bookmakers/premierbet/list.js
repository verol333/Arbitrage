// PremierBet — listing via bestsellers (1 seul appel, eventGames inclus).
...
export async function listMatches({ live = false, maxMatches = 200, horizonHours = 72 } = {}) {
  if (live) return []; // le live est servi par un autre backend, non intégré ici.
  const nowMs = Date.now();
  const horizonMs = nowMs + horizonHours * 3600 * 1000;
  const best = (await pget('market/events/bestsellers')) || [];
  const out = [];
  for (const e of best) {
    if (e.category1Name !== 'Soccer' || isOutright(e.eventName)) continue;
