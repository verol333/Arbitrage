...
const FAMILIES = {
  ...
  '2MT TT':       (k) => /^h2_tt_(home|away)_(over|under)_\d/.test(k),
  '2MT HCP':      (k) => /^h2_hcp_(home|away)_-?\d/.test(k),
  '2MT O/E':      (k) => /^h2_(odd|even)$/.test(k),
  'Corners Tot':  (k) => /^cor_(over|under)_\d/.test(k),
  'Corners HCP':  (k) => /^cor_hcp_(home|away)_-?\d/.test(k),
  'Corners O/E':  (k) => /^cor_(odd|even)$/.test(k),
  'Corners 1X2':  (k) => /^cor_match_[12X]$/.test(k),
  'Corners DC':   (k) => /^cor_dc_(1X|12|X2)$/.test(k),
  'Corners TT':   (k) => /^cor_tt_(home|away)_(over|under)_\d/.test(k),
  'Corners 1MT Tot': (k) => /^cor_ht_(over|under)_\d/.test(k),
  'Corners 1MT TT':  (k) => /^cor_ht_tt_(home|away)_(over|under)_\d/.test(k),
  'Corners 1MT HCP': (k) => /^cor_ht_hcp_(home|away)_-?\d/.test(k),
