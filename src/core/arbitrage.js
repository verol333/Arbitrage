...
export function compareTwoBooks(rawA, bookA, rawB, bookB) {
  ...
  // ─── VAGUE 1 : Clean Sheet, Pair/Impair par équipe, corners par équipe ──
  // Clean Sheet = binaire strictement complémentaire (l'équipe encaisse ou non).
  for (const [pfx, per] of [['', ''], ['ht_', '1MT '], ['h2_', '2MT ']]) {
    for (const [side, lbl] of [['home', 'Domicile'], ['away', 'Extérieur']]) {
  ...
  // Pair/Impair du total de buts d'une équipe (0 but compte comme pair).
  for (const [side, lbl] of [['home', 'Dom.'], ['away', 'Ext.']]) {
  ...
  // Total corners d'une équipe (plein temps et 1MT) — lignes dynamiques.
  for (const [pfx, per] of [['cor_tt_', 'Corners Total '], ['cor_ht_tt_', 'Corners 1MT Total ']]) {
    for (const [side, lbl] of [['home', 'Dom.'], ['away', 'Ext.']]) {
      for (const l of linesOf(oa, ob, new RegExp(`^${pfx}${side}_(?:over|under)_(\\d+(?:\\.\\d+)?)// Détection d'arbitrages (surebets) sur cotes plates standardisées.
// Port fidèle de matchCore.ts (pushArb, pushArb3, compareTwoBooks).
// Garde-fous conservés : cote > 80 ou profit > 40% → rejet (cotes gelées/corrompues).
import { config } from '../config.js';
import { normalizeAliases } from './markets.js';
