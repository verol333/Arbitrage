// Liste des matchs Maxibet — copie fidele de betmomo/list.js avec le SWARM
// scope au site_id=211. Meme structure de reponse (BetConstruct SWARM shared).
import { MAXIBET_SID, swarmSession, isOutright, isVirtual } from './api.js';

export async function listMatches({ live = false, maxMatches, horizonHours = 72, sport = 'football' } = {}) {
  const sid = MAXIBET_SID[sport];
  if (!sid) return [];
